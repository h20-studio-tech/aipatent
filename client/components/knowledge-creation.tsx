...
export default function KnowledgeCreation() {
  ...
  if (typeof window !== "undefined") {
    window.generateApproachInsights = () => {
      if (approachInsightsRef.current && approachInsightsRef.current.generateContent) {
        approachInsightsRef.current.generateContent()
      }
    }
    window.generateTechnologyInsights = () => {
      if (technologyInsightsRef.current && technologyInsightsRef.current.generateContent) {
        technologyInsightsRef.current.generateContent()
      }
    }
    window.generateInnovationInsights = () => {
      if (innovationInsightsRef.current && innovationInsightsRef.current.generateContent) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold tracking-tight">Knowledge Creation</h1>
        <p className="text-muted-foreground">Synthesize knowledge from existing sources to craft a new patent</p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Upload PDFs and chat with them to generate insights for your patent</CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="approach" className="w-full" onValueChange={(value) => setActiveTab(value)}>
            ...
            <TabsContent value="approach">
              <SectionPanel
                sectionId="approach"
                title="Approach"
                pdfs={pdfs.filter((pdf) => pdf.section === "approach" || !pdf.section)}
            ...
            <TabsContent value="technology">
              <SectionPanel
                ...
                title="Technology"
                pdfs={pdfs.filter((pdf) => pdf.section === "technology" || !pdf.section)}
                onPdfUpload={handlePdfUpload}
                hideInsights={true}
              />
            </TabsContent>
            <TabsContent value="innovation">
              <SectionPanel
                ...
                title="Innovation"
                pdfs={pdfs.filter((pdf) => pdf.section === "innovation" || !pdf.section)}
                onPdfUpload={handlePdfUpload}
                hideInsights={true}
              />
            </TabsContent>
      ...
      {activeTab === "approach" && <ApproachInsights ref={approachInsightsRef} />}
      {activeTab === "technology" && <TechnologyInsights ref={technologyInsightsRef} />}
      {activeTab === "innovation" && <InnovationInsights ref={innovationInsightsRef} />}
    </div>
  )
}
